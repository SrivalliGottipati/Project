...
class _CardsWithAPI2State extends State<CardsWithAPI2> {
  ...
  Widget build(BuildContext context) {
    return isLoading
        ...
        : CardRecommendUsers.isEmpty
            ? const Center(child: Text('No data available'))
            : ListOfCards(users: CardRecommendUsers);
  }
  ...
  Future<void> gettr2() async {
    const String url =
        'https://newsapi.org/v2/everything?q=everything&language=en&sortBy=publishedAt&from=2024-08-12&to=2024-08-25&apiKey=c5c9dfe1d21245f5bcf1d191d176b537';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          CardRecommendUsers =
              (json['articles'] as List<dynamic>).where((article) {
            final sourceId = article['source']['id'];
            final sourceName = article['source']['name'];
            return article['urlToImage'] != null && sourceId != null;
          }).toList();
        });
      }
    } catch (e) {
  ...
  ListView ListOfCards({required users}) {
    return ListView.builder(
        ...
        //   );
        // },
        itemCount: CardRecommendUsers.length);
  }
